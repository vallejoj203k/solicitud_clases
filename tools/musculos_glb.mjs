/**
 * Arma los GLB de músculos (client/public/modelo3d/musculos-*.glb) con lo que
 * deja tools/export_musculos.py en tools/build: simplifica la malla, la
 * compacta y la escribe comprimida con meshopt (EXT_meshopt_compression).
 *
 * Atributos de cada vértice (ver client/src/modelo3d/musculosAnatomicos.ts):
 *   POSITION  float32 x3  posición en la pose del cuerpo base (reposo)
 *   _ATADO    uint16  x4  triángulo del cuerpo, baricéntricas b1 y b2 (×65535), pieza
 *   _DESP     float32 x4  altura sobre la normal (m), residuo tangente en metros (c1, c2), tendón (0..1)
 *
 * Uso:  node tools/musculos_glb.mjs tools/build client/public/modelo3d
 */
import fs from 'node:fs';
import { MeshoptEncoder, MeshoptSimplifier } from 'meshoptimizer';

/** Triángulos que se buscan al simplificar (la malla de Z-Anatomy trae ~340 000). */
const TRIANGULOS_OBJETIVO = 140_000;
/** Error máximo de la simplificación, relativo al tamaño de la malla (~1,8 m). */
const ERROR_MAXIMO = 0.002;

const TIPOS = { '<f4': Float32Array, '<u4': Uint32Array, '<u2': Uint16Array };

function leerCrudo(carpeta, sexo) {
  const cab = JSON.parse(fs.readFileSync(`${carpeta}/musculos-${sexo}.json`, 'utf8'));
  const bin = fs.readFileSync(`${carpeta}/musculos-${sexo}.bin`);
  const a = {};
...
async function armar(entrada, salida, sexo) {
  ...
  orden.forEach((v, i) => {
    ...
    desp.set([a.desp[v * 3], a.desp[v * 3 + 1], a.desp[v * 3 + 2], a.tendon[v]], i * 4);
  });
  ...
  const json = {
    asset: { version: '2.0', generator: 'tools/musculos_glb.mjs' },
    ...
    meshes: [{ name: 'musculos', primitives: [{ attributes: { POSITION: 0, _ATADO: 1, _DESP: 2 }, indices: 3 }] }],
    nodes: [{ mesh: 0, name: 'musculos' }],
    scenes: [{ nodes: [0] }],
    scene: 0,
  };
  const etiqueta = sexo === 'M' ? 'hombre' : 'mujer';
  escribirGlb(`${salida}/musculos-${etiqueta}.glb`, json, Buffer.concat(partes));
  const meta = {
    version: 1,
    fuente: 'Z-Anatomy (CC BY-SA 4.0), derivado de BodyParts3D (CC BY-SA 2.1 Japón)',
    licencia: 'CC BY-SA 4.0',
    piezas: cab.piezas,
  };
  fs.writeFileSync(`${salida}/musculos-${etiqueta}.json`, JSON.stringify(meta));
  const mb = fs.statSync(`${salida}/musculos-${etiqueta}.glb`).size / 1e6;
  console.log(`musculos-${etiqueta}: ${cab.triangulos} -> ${indices.length / 3} triángulos, ${n} vértices, ${cab.piezas.length} piezas, ${mb.toFixed(2)} MB`);
}

const [, , entrada, salida] = process.argv;
if (!entrada || !salida) {
  console.error('Uso: node tools/musculos_glb.mjs tools/build client/public/modelo3d');
  process.exit(1);
}
await MeshoptSimplifier.ready;
await MeshoptEncoder.ready;
for (const sexo of ['M', 'F']) if (fs.existsSync(`${entrada}/musculos-${sexo}.bin`)) await armar(entrada, salida, sexo);
